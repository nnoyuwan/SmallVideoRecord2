...
//		TODO 开始录制的图标
        ...
        if (mHandler != null) {
            mHandler.removeMessages(HANDLE_INVALIDATE_PROGRESS);//移除刷新进度条Message
            mHandler.sendEmptyMessage(HANDLE_INVALIDATE_PROGRESS);//置空Message

            mHandler.removeMessages(HANDLE_STOP_RECORD);//移除延迟拍摄停止Message
            mHandler.sendEmptyMessageDelayed(HANDLE_STOP_RECORD,//距离结束录制的时间后置空Message
                    RECORD_TIME_MAX - mMediaObject.getDuration());
        }
//        mRecordDelete.setVisibility(View.GONE);
        mCameraSwitch.setEnabled(false);//禁止旋转摄像头
        mRecordLed.setEnabled(false);//禁止闪光灯
    }
            ...
            switch (msg.what) {
                case HANDLE_INVALIDATE_PROGRESS:
                    if (mMediaRecorder != null && !isFinishing()) {
                        if (mMediaObject != null && mMediaObject.getMediaParts() != null && mMediaObject.getDuration() >= RECORD_TIME_MAX) {
                            mTitleNext.performClick();
                            return;
                        }
                        if (mProgressView != null)
